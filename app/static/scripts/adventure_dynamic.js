   ...
   // Dynamic AJAX request for trip based on trip ID of button
	...
	$.ajax({
	  ...
	  success: function (users) {
 	    $('.trip-footer>.buddies').append('<P class="others modalfootertext">Other users on this trip:</P><DIV class="buddiesrow">');
	    for (user of users) {
	      $('.trip-footer>.buddies').append('<IMG src="' + user.profile_pic + '" class="img-circle rowperson" alt="' + user.first_name + '">');
	    }
